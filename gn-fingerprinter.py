...
pgrep = subprocess.check_output(["/usr/bin/pgrep","-u", "bbm-gn-fingerprinter", "-f", "/etc/bbm/bbm-gn-fingerprinter.yml"])
jpid = pgrep.rstrip("\n") 
if jpid == "":
   sys.exit(1)

# We can change over to tomcat7 user for secturity
utils.drop_privileges(user="bbm-gn-fingerprinter")

RunCollector(start_jmx_collector(15, jpid, jvm_collector + jetty_collector), extraTags=["application=gn-fingerprinter"])
